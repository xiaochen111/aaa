            ...
            <div>
                <div class="content-form">
                    ...
                    <div class="text-area border-bottom">
                        <textarea name="" id="" cols="30" v-model="notify.name" rows="10" v-checkParam="{dataType:'require',msg:'名称不能为空'}"></textarea>
                    </div>
                    <p class="label"> <span class="red">*</span>{{$t('common.address')}}：</p>
                    <div class="text-area border-bottom">
                        <textarea name="" id="" cols="30" v-model="notify.address" rows="10" v-checkParam="{dataType:'require',msg:'地址不能为空'}"></textarea>
                    </div>
                    <div class="item-filed border-bottom">
                        <p class="label">{{$t('common.Contact')}}：</p>
                        <div class="ipt">
                            <input type="text" v-model="notify.contact">   
                        </div>
                    </div>
                    <div class="item-filed border-bottom" @click="choisDef('countryIdCode')">
                        <p class="label">{{$t('common.countries')}}：</p>
                        <div class="ipt">
                            <input type="text" v-model="notify.countryIdCode" disabled='disabled'>
                        </div>
                    </div>
                    <div class="item-filed border-bottom">
                        <p class="label">{{$t('common.tel')}}：</p>
                        <div class="ipt">
                            <input type="text" v-model="notify.tel" v-checkParam="{dataType:'phone',msg:'电话格式不正确'}">
                        </div>
                    </div>
                    <div class="item-filed border-bottom">
                        <p class="label">{{$t('common.email')}}：</p>
                        <div class="ipt">
                            <input type="text" v-model="notify.eMail" v-checkParam="{dataType:'email',msg:'邮箱不正确'}">
                        </div>
                    </div>
                    <div class="item-filed border-bottom">
                        <p class="label">{{$t('common.postalcode')}}：</p>
                        <div class="ipt">
                            <input type="text" v-model="notify.postalCode">
                        </div>
                    </div>
                </div>
...
<script>
    ...
    import {notify} from '@/api/getData'
    import Vue from 'vue';
    export default {
        components:{
           navBar,Scroll,
        ...
        computed: {
            checked:{
                get:function(){
                    return this.notify.isDefault==1?true:false
                },
                set:function(newvalue){
                    this.notify.isDefault = newvalue
                }
            }
        },
        methods: {
            submitdef(){ // 新增or修改
                let id = this.$route.query.id;
                if(id){this.notify.id = id}
                this.notify.isDefault = this.checked ? '1' : '2';
                if(this.chiose.countryIdCode){this.notify.countryId = this.chiose.countryIdCode.id;}
                notify('saveOrUpdate',this.notify).then(res=>{
                    if(res.data.code=='1'){
                        history.go(-1);
                    }
                })
            ...
            // 修改
            eidtItem(params){
                notify('selectById',params).then(res=>{
